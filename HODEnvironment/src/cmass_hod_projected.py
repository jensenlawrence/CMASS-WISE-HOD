...
from halomod.integrate_corr import AngularCF, angular_corr_gal, projected_corr_gal, ProjectedCF
...
class AngularCrossCF(CrossCorrelations):
    ...
    def projected_corr_gal(self):
        rp = np.logspace(self.logu_min, self.logu_max, self.unum)
        rlim = 100.0
        # np.savetxt('r.txt',self.halo_model_1.r)
        # print('redshift',self.halo_model_1.z)
        # np.savetxt('corr_cross.txt',self.corr_cross-1.0)
        return projected_corr_gal(self.halo_model_1.r, self.corr_cross-1.0, rlim, rp)

# ----------------------------------------------------------------------------------------------------------------------
# CMASS_HOD Class
# ----------------------------------------------------------------------------------------------------------------------

class CMASS_HOD_PROJECTED:
    """
    ...
    def __init__(self, cmass_redshift_file, data_file, covariance_file, params_file,
        ...
        cmass_nz = np.loadtxt(cmass_redshift_file)[:,1]
        cmass_z = np.linspace(0, 4.00, 28)
        ...
        # self.cmass_auto = self.cmass_auto()
    ...
    def __eq__(self, other):
        ...
        """
        are_equal = isinstance(other, CMASS_HOD_PROJECTED) and (self.summary == other.summary)
        return are_equal
    ...
    def cmass_auto(self):
        ...
        auto = ProjectedCF(
            rp_min = np.min(self.data[:,0])-0.5 * np.gradient(self.data[:,0])[0],
            rp_max = np.max(self.data[:,0])+0.5 * np.gradient(self.data[:,0])[-1],
            rp_num = 100*self.data[:,0].size,
            rp_log = True,
            hod_model = 'Zheng05',
    ...
    def corr_cmass_auto(self, update_cmass_params={}):
        ...
        # Calculate projected autocorrelation
        auto_corr = cmass_auto.projected_corr_gal_rsd
    ...
    def loglike(self, cmass_M_min, cmass_M_1, cmass_alpha, cmass_M_0, cmass_sig_logm, wise_M_min, wise_M_1,
        ...
        wp_theory_binned = np.zeros(len(self.data[:,0]))
        ...
        bins_rp = np.logspace(bin_zero_min, bin_last_max, len(self.data)+1)
        cmass_auto_corr = np.zeros(len(bins_rp)-1)
        wpspline = _spline(self.cmass_auto().rp, cmass_auto_corr_big)
        for i in range(len(wp_theory_binned)):
            rp_big = np.linspace(bins_rp[i],bins_rp[i+1],1000)
            cmass_auto_corr[i] = (np.sum(2 * rp_big * (1 + wpspline(rp_big))*np.gradient(rp_big))/((bins_rp[i+1]**2.-bins_rp[i]**2.))-1.)
        
        print(f'rp = {0.5 * (bins_rp[1:] + bins_rp[:-1])}') 
