...
def crosscorr_plot(cmass_wise_hod, sampled=[], plot_title='', output='', dpi=200):
    ...
    # Plot holding all parameters fixed
        ...
        plt.plot(cmass_wise_hod.thetas[:-1], cmass_wise_hod.corr_cross(), color='dodgerblue', label='Model')
    ...
    plt.errorbar(cmass_wise_hod.thetas[:-1], cmass_wise_hod.data[7:-1,1], np.sqrt(np.diag(cmass_wise_hod.covariance[7:-1,7:-1])),
                 color='springgreen', label='Cross-correlation Data')
...
def get_corr_title(params, loglike_func):
    ...
    cmass_s1 = r'$M_{\min} = $' + f'{round(params["CMASS HOD"]["M_min"]["val"], 2)}'
    cmass_s2 = r'$M_{1} = $' + f'{round(params["CMASS HOD"]["M_1"]["val"], 2)}'
    cmass_s3 = r'$\alpha = $' + f'{round(params["CMASS HOD"]["alpha"]["val"], 2)}'
    cmass_s4 = r'$M_{0} = $' + f'{round(params["CMASS HOD"]["M_0"]["val"], 2)}'
    cmass_s5 = r'$\sigma_{\log{M}} = $' + f'{round(params["CMASS HOD"]["sig_logm"]["val"], 2)}'
    ...
    wise_s1 = r'$M_{\min} = $' + f'{round(params["WISE HOD"]["M_min"]["val"], 2)}'
    wise_s2 = r'$M_{1} = $' + f'{round(params["WISE HOD"]["M_1"]["val"], 2)}'
    wise_s3 = r'$\alpha = $' + f'{round(params["WISE HOD"]["alpha"]["val"], 2)}'
    wise_s4 = r'$M_{0} = $' + f'{round(params["WISE HOD"]["M_0"]["val"], 2)}'
    wise_s5 = r'$\sigma_{\log{M}} = $' + f'{round(params["WISE HOD"]["sig_logm"]["val"], 2)}'
    ...
    likelihood_title = f'Log-likelihood = {round(likelihood, 2)}'
