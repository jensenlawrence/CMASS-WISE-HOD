    ...
    **kwargs: anything that can be used in the MassFunction class
    ...
    def halo_profile_model(self, val):
        ...
        if not (isinstance(val, str) or issubclass_(val, profiles.Profile)):
            ...
            return get_model_(val, "profiles")
        else:
            return val
    ...
    def halo_concentration_model(self, val):
        ...
        if not (isinstance(val, str) or issubclass_(val, CMRelation)):
            ...
            return get_model_(val, "concentration")
        else:
            return val
    ...
    def exclusion_model(self, val):
        ...
        if issubclass_(val, Exclusion):
            ...
            return get_model_(val, "halo_exclusion")
...
class TracerHaloModel(DMHaloModel):
    ...
    def mean_tracer_den(self):
        ...
        print('mean_tracer_den: integrand',time.time()-t0)
        out = intg.simps(integrand, self.m[self._tm])
        print('mean_tracer_den: out',time.time()-t0)
        return out
    ...
    def _power_2h_auto_tracer_primitive(self):
        ...
        #print('phh')
        xi_hh = tools.hankel_transform(self._power_halo_centres_fnc, self._r_table, "r", h=1e-4)
        #np.savetxt('/home/akrolews/xi_hh.txt',np.array([self._r_table, xi_hh]).T)
        #np.savetxt('/home/akrolews/phh.txt',np.array([self.k,phh]).T)
        #print('self.transfer.cosmo',self.transfer.cosmo)
        #print('self.transfer.takahashi',self.transfer.takahashi)
        #print('self.transfer.sigma8',self.transfer.sigma8)
        #print('self.sigma_8',self.sigma_8)
        #print('self.n',self.n)
        #print('self.transfer_model',self.transfer_model)
        #print('type of nonlinear',type(self.nonlinear_power))
        # np.savetxt('/home/akrolews/pnl.txt',np.array([self.k,self.nonlinear_power_fnc(self.k)]).T)
        #print(self.cosmo)
        #print(self.z)
        #print(self.sigma8)
        #print(self.takahashi)
        if intg.ndim == 2:
            p = []
    ...
    def power_2h_auto_tracer(self):
        ...
        if np.any(self.k_hm < 1e-2):
            ...
            #print('shape power',np.shape(self.power))
            #print('shape k_hm',np.shape(self.k_hm))
            out[self.k_hm < 1e-2] = (
                self.linear_power_fnc(self.k_hm)[self.k_hm < 1e-2] * self.bias_effective_tracer**2.
            )
    ...
    def power_auto_tracer(self):
        ...
        #print('in the power')
        #np.savetxt('power_1h_auto_tracer_fnc.txt',self.power_1h_auto_tracer_fnc(self.k_hm))
        #np.savetxt('power_2h_auto_tracer.txt',self.power_2h_auto_tracer)
        t0 = time.time()
        out = self.power_1h_auto_tracer_fnc(self.k_hm) + self.power_2h_auto_tracer
        # print('power_auto_tracer',time.time()-t0)
        return out

    @cached_quantity
    def corr_auto_tracer_fnc(self):
        """The tracer auto correlation function"""
        #print('corr_auto_tracer_fnc')
