...
def wp(b1, b2, zmin, zmax, pairwise=True):
	...
	for ell in ells:
		...
		np.savetxt('power_auto_halofit.txt',np.array([k, p_auto]))
		xi_ell_dir = np.real((1j)**ell * 1./(2*np.pi**2.) * np.sum(dk * k**2 * p_auto * spherical_jn(ell, k * r[:,np.newaxis]),axis=1))
		ht = HankelTransform(nu = ell + 0.5,
			N = 1200,
			h = 0.005)
		power_func = lambda k: pk(pars, k, 0.5*(zmin + zmax))[0] * k**0.5 * np.sqrt(np.pi/2) * 1. /(2*np.pi**2.)
		xi_ell = np.real((1j)**ell * ht.transform(power_func, r, ret_err=False)/r**0.5)
		print(xi_ell/xi_ell_dir)
		np.savetxt('halofit_xi_%i.txt' % ell, xi_ell)
		np.savetxt('halofit_r.txt',r)
...
#bias = 2.071
bias = 2.11678
rp, wpout = wp(bias, bias, 0.5472, 0.5472)
...
chi2 = np.sum(((wp_data-(wp_theory_binned[4:]-integral_constraint))**2./np.diag(covariance_file)[2:])[6:])
#print(chi2s[j])

np.savetxt('halofit_bias=2.11678_hankel.txt',wp_theory_binned)