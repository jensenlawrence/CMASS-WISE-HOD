...
def angular_corr_gal_rsd(
    ...
    p2=None, check_p_norm=True, cosmo=None, p_of_z=True, **xi_kw
):
    if cosmo is None:
        cosmo = csm().cosmo
    ...
    #np.savetxt('r.txt',r)
    #np.savetxt('xir_0.txt',xir_0)
    #np.savetxt('xir_2.txt',xir_2)
    #np.savetxt('xir_4.txt',xir_4)
    ...
    #np.savetxt('xi_rsd.txt',xi_rsd(R,mu))
    #np.savetxt('p_integ.txt',p_integ)
    #np.savetxt('p1x.txt',p1(x))
    #np.savetxt('p2x.txt',p2(x))
    #np.savetxt('x.txt',x)

    integrand = np.einsum(
        "kij,i->kij", xi_rsd(R, mu).reshape((len(theta), len(x), len(u))), p_integ
    )
    #np.savetxt('integrand.txt',integrand.flatten())
