...
class AngularCrossCF(CrossCorrelations):
    ...
    def angular_corr_gal(self):
        def xi(r):
            ...
            return s(r)
        r = np.linspace(1,600,100)
        xr = xi(r)
        np.savetxt('xir.txt',xr)
...
class CMASS_WISE_HOD(AngularCrossCF):
    ...
    def __init__(self, cmass_redshift_file, wise_redshift_file, data_file, covariance_file, params_file,
                 cross_hod_model, diag_covariance=False, exclusion_model=None, exclusion_params=None, thetamin=0, thetamax=0, numtheta=0):
        """
        ...
        np.savetxt('cmass_zfunc.txt',np.array([chirange,cmass_zfunc(chirange)]).T)
    ...
    def corr_cmass_auto(self, update_cmass_params={}):
        ...
        if update_cmass_params != {}:
            print('\n')
            print('CMASS parameters before update', cmass_auto.halo_model_1.hod_params)
            ...
            print('CMASS parameters after update', cmass_auto.halo_model_1.hod_params)


        # Calculate angular cross-correlation
        auto_corr = cmass_auto.angular_corr_gal
        print('cross_corr',auto_corr)
        return auto_corr
    ...
    def loglike_components(self, cmass_M_min, cmass_M_1, cmass_alpha, cmass_M_0, cmass_sig_logm, wise_M_min, wise_M_1,
                           wise_alpha, wise_M_0, wise_sig_logm, R_ss, R_cs, R_sc, testing=False):
        """
        ...
        testing: bool, optional
            if testing = True, stop evaluation after computing cross-correlation & auto-correlation.
            i.e. don't compute likelihoods.
            if testing = False, compute likelihoods and go through everything.
        ...
        print('cmass_params',cmass_params)
        ...
        if testing == True:
            ...
            loglike_dict = {
                'auto_loglike': np.nan,
                'cross_loglike': np.nan,
                'total_loglike': np.nan
            }
