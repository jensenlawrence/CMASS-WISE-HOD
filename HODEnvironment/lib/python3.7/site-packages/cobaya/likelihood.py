...
class Likelihood(Theory, LikelihoodInterface):
    ...
    def calculate(self, state, want_derived=True, **params_values_dict):
        ...
        logp = self.logp(_derived=derived, **params_values_dict)
        state["logp"] = logp[0]
        self.log.debug("Computed log-likelihood = %g", state["logp"])
        print('fucking params values dict',params_values_dict)
        print('fucking derived',derived)
        #if want_derived:
        print('fucking logp1',logp[1])
        state["derived"] = logp[1]
...
class LikelihoodExternalFunction(Likelihood):
    def __init__(self, info, name, timing=None):
        ...
        # END OF DEPRECATION BLOCK
        print('fuck fuck fuck',info.get(_output_params, []))
        print('fucking shitter fucker fucker',_output_params)
        print('mother fucker god damn it',type(info))
        print('god damned shitter shithead',info)
    ...
    def logp(self, **params_values):
        ...
        try:
            ...
            print('goddamn shitfucking shitfuck cocksuck',return_value)
            if hasattr(return_value, "__len__"):
                logp = return_value[0]
                print('motherfucker',self.output_params)
                self.output_params = ['nbar_cmass','nbar_wise', 'bias_cmass', 'bias_wise', 'avg_mass_cmass', 'avg_mass_wise', 'sat_frac_cmass', 'sat_frac_wise']
                if self.output_params:
                    try:
                        ...
                        print('225')
                        params_values["_derived"] = _derived
                        print('cocksuck shit',_derived)
                        print('fuck you asshole',params_values)
                    except:
